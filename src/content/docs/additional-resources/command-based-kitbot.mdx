...
Use git to fork and clone the template code.
...
Create a new package named `subsystem` under the `robot` package.
Remember that packages are folders that contain java files.
The `subsystem` package will contain the code for our feeder and intake launcher subsystems.

Then, create a new Java class in the `subsystems` package named `Feeder.java`, which will be used to control the feeder.
The `Feeder` class should implement `Subsystem`.

<details class="accordion">
    <summary>Hint</summary>
    A subsystem is defined as a class with the `implements Subsystem` keyword at the end.
    ```java #feederDef
...
<details class="accordion">
    ...
    To set a subsystem's default command, call `setDefaultCommand(Command)`
...
### Task 3: The IntakeLauncher Subsystem

Create a file named `IntakeLauncher.java` in the `subsystem` package.
Define a class named `IntakeLauncher` that implements `Subsystem`; then, add the following:
...
Now that we have the subsystem classes created, we need to instantiate them in the `Robot` class.
