...
For robots, we want our program to do something similar.
This could be "If the timer starts, drive the robot for 5 seconds then stop when the timer reached 15 seconds.
...
Sometimes we want our code to have specific logic for multiple cases when deciding.
...
Our code above has the robot drive at full speed if the distance is greater than 10cm, has the robot drive at half speed if the distance is less than 10cm, but the robot never stops if it reaches its target distance.
