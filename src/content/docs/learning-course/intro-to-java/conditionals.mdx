...
<Aside type="note">
    There is also another type of conditional, called switch, but we will not be covering it in this section
    because it is not used as frequently in FRC programming.
    However, if you want to learn more about the switch statement, you can learn
...
This means the code inside the `if` statement runs, which prints "Motors are spinning" and the spins the motors at half speed.
...
Here's how this works: if `if` statement's condition is `true`, then the the code inside will run.
