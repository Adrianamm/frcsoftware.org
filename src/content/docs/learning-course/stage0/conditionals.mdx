...
For example, "If we are hungry, then we get a snack" or "If we are tired, we go to bed."
Robots often need to make different decisions based on different situations.
...
<Aside type="note">
    There is also another type of conditional called a switch statement. They
    aren't common in FRC and therefore will not be covered. To learn more about
    switch statements, you can read about them
    [here.](https://www.w3schools.com/java/java_switch.asp)
</Aside>
...
A code block a code block is a sequence of code.
...
In this example, this code will move forward only when distance is smaller then 10.

```java #ifExample

```

This works because the condition is `distance < 10`.
...
In this example, we now have an `else` statement that sets the motor speed to 0 if `distance` is greater than 20.
...
An `else if` statement let you choose between as many conditions as you like.
