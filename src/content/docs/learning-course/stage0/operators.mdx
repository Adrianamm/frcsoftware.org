...
<Aside type="note">
    When dividing two integers, the result will be a rounded down integer instead of a
    ...
    Due to this, it's better to use `double`s when writing code because it's more precise.
...
<Aside type="note">
`==` and `!=` can not be used to compare `String`s. 
