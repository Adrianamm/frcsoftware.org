...
There are also some rules with variable names. The name of the variable can not include spaces. Instead you can write variables with camel case (frontLeftDrive). Snake camel case (front_Left_Drive) is used for constants which we will learn about later on in the course.
Variable names can have numbers any point in the name but it can not start with a number.
